...
// Lebih aman: hindari ambil id_user dari GET
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($password !== $konfirmasi_password) {
        ...
        if ($validation !== true) {
            ...
            $result = changePassword($koneksi, $hashedPassword, $id_user);
            if ($result === true) {
                header("Location: login.php?reset=truee");
                exit();
            } else {
    ...
    <div class="bg-white shadow-md rounded-xl p-8 w-full max-w-md m-4">
      ...
      <form method="POST" class="space-y-4">
        <div>
          ...
          <input id="password" type="password" required name="password" class="input input-bordered w-full rounded-lg" />
          <p class="text-sm text-gray-500 mt-1">
            Password must be at least 8 characters, include uppercase, lowercase, a number, and a special symbol (e.g., !@#$%^&*).
          </p>
        </div>
        <div>
